(function () {
    ...
    t.init = function () {
        ...
        if (!localStorage.measure) {
            localStorage.measure = "mh"; // Default unit to miles per hour (mph)
        }
        ...
        t.writeSpeed = function (position) {
            ...
            // Convert speed to miles per hour (mph) or kilometers per hour (km/h)
            if (localStorage.measure === "kh") {
                // Convert meters per second to kilometers per hour (km/h)
                speed = 60 * speed * 60; // km/h
                speed /= 1000; // Convert to km/h
                ...
                speed = 60 * speed * 60; // mph
                speed *= 0.000621371192; // Conversion factor from meters per second to miles per hour
                document.getElementById("unit").innerHTML = "miles/hour"; // Display unit as miles/hour
            }
